...
def test_confluence_page_compact(snapshot: SnapshotAssertion) -> None:
    """atls confluence page get 12345678 --format=compact matches snapshot."""
    respx.get(f"{CONFLUENCE_URL}/rest/api/content/12345678").mock(
        return_value=httpx.Response(200, json=_load("get-page-12345678.json")["metadata"])
    )
    result = runner.invoke(app, ["confluence", "page", "get", "12345678", "--format", "compact"])
...
def test_confluence_page_json_format(snapshot: SnapshotAssertion) -> None:
    """atls --format=json confluence page get 12345678 matches snapshot."""
    respx.get(f"{CONFLUENCE_URL}/rest/api/content/12345678").mock(
        return_value=httpx.Response(200, json=_load("get-page-12345678.json")["metadata"])
    )
